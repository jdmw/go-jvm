...
public class AttrStructFile {
    ...
    public AttrStructFile(String name) {
        this.name = name;
    }
    ...
    public String toString(){
        ...
        attrs.forEach((name,type)->{
            sb.append("\t\t").append(name).append("\t").append(type).append("\n");
        });

        finish = true ;
        // constructor
        sb.append("\t}\n\tfunc (self *").append(name).append(") parse(cf ClassFile,length u4,r *BigEndianReader) {\n")
          .append("\t\tself.cp = cf.constant_pool\n");

        attrs.forEach((n,type)->{
            if(type.startsWith("u")){
                sb.append("\t\tself.").append(name).append(" = r.ReadI").append(type.substring(1)).append("()\n");
            }
        });
        sb.append("\t}\n\n");
